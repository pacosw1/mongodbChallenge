...
export const AssertEqual = (original, created): boolean => {
    ...
    for (key of createdKeys) {
        ...
        if (nestedKeys.length === 1) {
            if (original[key] !== created[key]) return false
        ...
        for (let nestedKey of nestedKeys) {
            if (!(nestedKey in object)) return false
            object = object[nestedKey]
        }
        if (object !== created[key]) return false
